...
import { configureEvent } from '../../app/admin/actions'
...
import type { EventConfig, Preset } from '../../lib/config'
...
// Guesses which curated font pair an existing config's fonts match, falling
// back to the first pair when editing a config saved before this feature
// existed (or a combination the wizard's curated list no longer offers).
function findFontPairIdFor(fonts: EventConfig['fonts']): FontPairId {
  const match = FONT_PAIRS.find((f) => f.display === fonts.display && f.body === fonts.body)
...
export function WizardForm({ eventId, initialEventName, initialConfig }: { eventId: string; initialEventName: string; initialConfig: EventConfig | null }) {
  const router = useRouter()
  const isEditing = initialConfig !== null

  const [presetId, setPresetId] = useState(PRESET_NAMES[0])
  const [eventName, setEventName] = useState(toEditable(initialEventName))
  const [colors, setColors] = useState(initialConfig?.colors ?? PRESETS[PRESET_NAMES[0]].colors)
  const [fontPairId, setFontPairId] = useState<FontPairId>(
    initialConfig ? findFontPairIdFor(initialConfig.fonts) : FONT_PAIRS[0].id
  )
  const [texts, setTexts] = useState(toEditableTexts(initialConfig?.texts ?? PRESETS[PRESET_NAMES[0]].texts))
  const [qrColor, setQrColor] = useState(initialConfig?.qrColor ?? PRESETS[PRESET_NAMES[0]].colors.primary)
  const [qrColorTouched, setQrColorTouched] = useState(isEditing)
  const [titleEffect, setTitleEffect] = useState<TextEffectId>((initialConfig?.titleEffect as TextEffectId) ?? 'ninguno')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      const { slug } = await configureEvent(eventId, {
        preset: presetId,
  ...
  return (
    <main style={{ maxWidth: 920, margin: '3rem auto', fontFamily: 'system-ui, sans-serif', padding: '0 1.5rem' }}>
      <h1>{isEditing ? 'Editar diseño del evento' : 'Configurar evento'}</h1>
      {isEditing && (
        <p style={{ fontSize: '0.85rem', color: '#888', marginTop: '-0.6rem', marginBottom: '1.4rem' }}>
          El link del evento y las fotos ya subidas no cambian — solo se actualiza la estética.
        </p>
      )}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', alignItems: 'start' }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <button type="submit" disabled={submitting}>
            {submitting ? 'Guardando…' : isEditing ? 'Guardar cambios' : 'Generar evento'}
          </button>
